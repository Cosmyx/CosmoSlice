...
// ---------------------------------------------------------------------------
// CosmyxPrintHostSendDialog – webview-based send dialog, Cosmyx printers only
// ---------------------------------------------------------------------------

static wxString html_escape(const wxString& s)
{
    wxString o;
    o.reserve(s.length() * 11 / 10);
...
CosmyxPrintHostSendDialog::CosmyxPrintHostSendDialog(
...
void CosmyxPrintHostSendDialog::init()
{
    // Hide the standard controls created by the base constructor
    if (txt_filename)  txt_filename->Show(false);
    if (combo_groups)  combo_groups->Show(false);
    if (combo_storage) combo_storage->Show(false);

    // Locate the template files
    fs::path base_dir  = fs::path(Slic3r::resources_dir()) / "web" / "print_host";
    ...
    wxString html;
    {
        wxFile f(wxString::FromUTF8(html_path.string().c_str()));
        if (f.IsOpened())
            f.ReadAll(&html);
        else
            html = "<html><body style='background:#2d2d31;color:#e0e0e0;font-family:sans-serif;padding:20px'>"
                   "<p>Missing resources/web/print_host/index.html</p></body></html>";
    }

    // Inline the CSS so it works regardless of the webview's base-URL handling
    {
        wxFile css_f(wxString::FromUTF8(css_path.string().c_str()));
        if (css_f.IsOpened()) {
            wxString css;
            css_f.ReadAll(&css);
            wxString css_block = "<style>\n" + css + "\n</style>";
            html.Replace("<link rel=\"stylesheet\" href=\"style.css\">",  css_block, false);
            html.Replace("<link rel=\"stylesheet\" href=\"style.css\"/>", css_block, false);
        }
    }

    // Build the pre-filled filename (same logic as base class)
    AppConfig* app_config = wxGetApp().app_config;
    wxString   recent_path = from_u8(app_config->get("recent", CONFIG_KEY_PATH));
    if (!recent_path.empty() && recent_path.Last() != '/')
        recent_path += '/';
    recent_path += wxString(m_path.filename().wstring());

    // Restore persisted "Upload and Print" checkbox state
    const bool saved_c1 = app_config->get_bool("cosmyx_upload_and_print");

    // Replace template placeholders
    html.Replace("{TITLE}",            _L("Send G-code to printer host"), false);
    html.Replace("{FILENAME_LABEL}",   _L("Filename"),                    false);
    html.Replace("{PLACEHOLDER}",      _L("(generated by OrcaCosmyx)"),   false);
    html.Replace("{FILENAME}",         html_escape(recent_path),          false);
    html.Replace("{AUTO_PRINT_LABEL}", _L("Upload and Print"),            false);
    html.Replace("{CANCEL_LABEL}",     _L("Cancel"),                      false);
    html.Replace("{VALIDATE_LABEL}",   _L("Upload"),                      false);
    html.Replace("{C1}",               saved_c1 ? "checked" : "",         false);

    // base URL for the webview (resolves relative resources if any are ever added)
    wxString base_url = "file://" + wxString::FromUTF8(base_dir.string().c_str());
#ifdef __WIN32__
    base_url.Replace("\\", "/");
    ...
    // Create the webview
    auto* web = WebView::CreateWebView(this, "about:blank");
    web->SetMinSize(FromDIP(wxSize(700, 420)));

    // Inject HTML once the blank page has loaded (avoids SetPage-before-ready issues)
    auto loaded = std::make_shared<bool>(false);
    web->Bind(wxEVT_WEBVIEW_LOADED, [web, html, base_url, loaded](wxWebViewEvent&) {
        if (!*loaded) {
            *loaded = true;
    ...
    // Intercept app:// navigation events to receive form submissions
    web->Bind(wxEVT_WEBVIEW_NAVIGATING, [this, app_config](wxWebViewEvent& e) {
        ...
        if (url.StartsWith("app://submit")) {
            ...
            app_config->set_bool("cosmyx_upload_and_print", c1 == "1");

            // Write the filename into the hidden control so the base EndModal can read it
            if (!fname.IsEmpty())
                txt_filename->SetValue(fname);

            post_upload_action = (c1 == "1")
        ...
        else if (url.StartsWith("app://cancel")) {
            e.Veto();
            CallAfter([this] {
    ...
    content_sizer->Add(web, 1, wxEXPAND);
    finalize();
}
