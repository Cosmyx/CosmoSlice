...
void SendDeviceItem::doRender(wxDC& dc)
{
    ...
    //device state
    if (state_printable <= 2) {
        dc.SetTextForeground(wxColour(156, 39, 176));
    }
    else {
    ...
    if (m_hover) {
        dc.SetPen(wxPen(wxColour(156, 39, 176)));
...
wxPanel* SendMultiMachinePage::create_page()
{
    ...
    // add send button
    btn_bg_enable = StateColor(std::pair<wxColour, int>(wxColour(0, 137, 123), StateColor::Pressed), std::pair<wxColour, int>(wxColour(171, 71, 188), StateColor::Hovered),
        std::pair<wxColour, int>(wxColour(156, 39, 176), StateColor::Normal));
