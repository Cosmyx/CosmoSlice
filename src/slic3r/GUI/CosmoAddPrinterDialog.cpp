...
{
    BOOST_LOG_TRIVIAL(warning) << "[CosmoLink] CosmoAddPrinterDialog constructor start (parent=" << (parent ? "valid" : "null") << ")";
    ...
    {
        BOOST_LOG_TRIVIAL(warning) << "[CosmoLink] Dialog: loading printer presets (preset_bundle=" << (wxGetApp().preset_bundle ? "valid" : "null") << ")";
        ...
        BOOST_LOG_TRIVIAL(warning) << "[CosmoLink] Dialog: current preset='" << current_name << "'";
        int sel = 0, idx = 0;
        for (const Preset& p : printers.get_presets()) {
            if (!p.is_visible || p.is_default)
                continue;
            BOOST_LOG_TRIVIAL(warning) << "[CosmoLink] Dialog: adding preset '" << p.name << "'";
            m_choice_preset->Append(wxString::FromUTF8(p.name));
            if (p.name == current_name)
                sel = idx;
            ++idx;
        }
        BOOST_LOG_TRIVIAL(warning) << "[CosmoLink] Dialog: " << idx << " preset(s) loaded, selection=" << sel;
    ...
    CentreOnParent();
    BOOST_LOG_TRIVIAL(warning) << "[CosmoLink] CosmoAddPrinterDialog constructor complete";
}
