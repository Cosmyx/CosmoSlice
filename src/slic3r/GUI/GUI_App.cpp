...
//        if (wParam == RIM_INPUTSINK && plater != nullptr && main_frame->IsActive()) {
	...
	wxWindow::MSWRegisterMessageHandler(WM_COPYDATA, [](wxWindow* win, WXUINT /* nMsg */, WXWPARAM wParam, WXLPARAM lParam) {
		BOOST_LOG_TRIVIAL(warning) << "[CosmoLink] WM_COPYDATA received";
		COPYDATASTRUCT* copy_data_structure = { 0 };
		copy_data_structure = (COPYDATASTRUCT*)lParam;
		if (copy_data_structure->dwData == 1) {
			LPCWSTR arguments = (LPCWSTR)copy_data_structure->lpData;
			std::string msg = boost::nowide::narrow(arguments);
			BOOST_LOG_TRIVIAL(warning) << "[CosmoLink] WM_COPYDATA message='" << msg << "'";
...
void GUI_App::post_init()
{
    ...
    BOOST_LOG_TRIVIAL(warning) << "[CosmoLink] post_init() called. input_files count=" << this->init_params->input_files.size();
    ...
    if (!this->init_params->input_files.empty()) {
        ...
        BOOST_LOG_TRIVIAL(warning) << "[CosmoLink] post_init: first input file = '" << first_url << "'";
        if (this->init_params->input_files.size() == 1 && is_cosmoslice_url(first_url)) {
            BOOST_LOG_TRIVIAL(warning) << "[CosmoLink] post_init: detected cosmoslice URL, calling handler";
            switch_to_3d = true;
